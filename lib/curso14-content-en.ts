...
b) It allows LLMs to access external information that wasn't in their training
c) It reduces the cost of training language models
d) It improves the grammar of generated responses

**Correct answer: b)** RAG solves the problem of LLMs' limited knowledge, allowing them to access up-to-date and private information that wasn't part of their training, thus reducing hallucinations.
...
a) Embeddings -> Chunking -> Search -> Generation
b) Chunking -> Search -> Embeddings -> Generation
c) Chunking -> Embeddings -> Search -> Generation
d) Search -> Chunking -> Embeddings -> Generation

**Correct answer: c)** The correct pipeline is: first documents are split into chunks, then embeddings (vectors) are generated for each chunk, then the most similar chunks to the query are found, and finally the LLM generates a response with that context.
...
a) The number of documents processed in parallel
b) The text overlap between consecutive chunks to avoid losing context at boundaries
c) The number of times the search is repeated
d) The minimum distance between embeddings to consider them similar

**Correct answer: b)** Overlap is the amount of text shared between consecutive chunks. It prevents important information that falls at the boundary between two chunks from being lost, ensuring context continuity.
...
a) Text fragments extracted from documents
b) Instructions given to the LLM
c) Numerical representations (vectors) of text that capture its semantic meaning
d) Relational database indexes

**Correct answer: c)** Embeddings are vectors of numbers (typically 384 to 1536 dimensions) that represent the semantic meaning of text. Texts with similar meanings will have close embeddings in vector space.
...
a) RAG produces longer responses
b) RAG allows updating information without re-training the model and offers source traceability
c) RAG is always more accurate than fine-tuning
d) RAG doesn't need any kind of pre-processing

**Correct answer: b)** RAG allows updating data instantly (just add documents to the index), cites the sources where it got the information, keeps private data separate from the model, and is significantly cheaper and faster than re-training.
...
a) A metric for measuring the quality of LLM responses
b) A technique that balances relevance with diversity in search results
c) An algorithm for compressing embeddings
d) A method for merging multiple vector stores

**Correct answer: b)** MMR selects results that are relevant to the query but also diverse from each other. This avoids retrieving multiple redundant fragments that say the same thing, maximizing the useful information the LLM receives.
...
a) Whether the retrieved documents are relevant to the question
b) Whether the generated response is consistent with the retrieved context (doesn't hallucinate)
c) Whether the user is satisfied with the response
d) Whether the embedding model is accurate

**Correct answer: b)** Faithfulness measures whether the claims in the generated response are supported by the retrieved context. Low faithfulness indicates the LLM is inventing information that isn't in the documents (hallucinating).
...
a) Two different LLM models
b) Keyword search (BM25) with semantic (vector) search
c) Large chunks with small chunks
d) OpenAI embeddings with Hugging Face embeddings

**Correct answer: b)** Hybrid search combines keyword search (like BM25, which looks for exact word matches) with semantic search (vector-based, which understands meaning). This is especially useful because semantic search can fail with technical terms or specific codes where exact matching is better.

---

### Question 9

What is the "parent-child chunking" technique?

a) Creating chunks for main and secondary documents separately
b) Using small chunks for precise search but retrieving the parent chunk (larger) to give more context to the LLM
c) Splitting a document first by chapters and then by paragraphs
d) Indexing both the original document and its summary

**Correct answer: b)** Parent-child chunking creates two levels: small child chunks used for search (because they're more specific and give better matching precision), and large parent chunks sent to the LLM (because they provide more context for generating complete responses).

---

### Question 10

You have a RAG system with high "answer relevancy" but low "faithfulness." What's happening?

a) The system isn't finding relevant documents
b) The responses are relevant to the question but contain invented information that isn't in the documents
c) The LLM is too slow
d) The chunks are too large

**Correct answer: b)** High relevancy with low faithfulness means the LLM understands what it's being asked and generates responses that seem appropriate, but it's adding information that doesn't come from the retrieved context (it's hallucinating). Solutions include: improving the prompt to be stricter with the context, reducing the LLM's temperature, and improving retrieval to provide better context.
`;
