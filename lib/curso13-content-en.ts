...
Test your course knowledge with these 10 multiple choice questions. Each question has one correct answer.
...
a) \`prompt = f"Hello {name}. \\n Topic: {topic}"\`
b) \`prompt = f"""Hello {name}. Topic: {topic}"""\`
c) \`prompt = "Hello " + name + ". Topic: " + topic\`
d) \`prompt = format("Hello {}, Topic: {}", name, topic)\`

**Correct answer: b)** — Triple quotes (\`f"""..."""\`) allow you to create multiline f-strings while maintaining format and variable interpolation, which is ideal for complex AI prompts.
...
a) Because Python doesn't work without a virtual environment
b) Because it makes code run faster
c) Because it isolates each project's dependencies, preventing version conflicts
d) Because it's a security requirement of AI APIs

**Correct answer: c)** — Virtual environments create isolated Python installations. Without them, different projects might require incompatible versions of the same libraries, causing hard-to-diagnose errors.

---

### Question 3

What is the main difference in how the system prompt is passed between the OpenAI and Anthropic APIs?

a) OpenAI doesn't support system prompts
b) In OpenAI it goes as a message with role "system" in the list; in Anthropic it's a separate parameter
c) In Anthropic it goes as a message with role "system" in the list; in OpenAI it's a separate parameter
d) Both APIs handle the system prompt in the same way

**Correct answer: b)** — In OpenAI, the system prompt is included as the first message with \`{"role": "system", "content": "..."}\`. In Anthropic, it's passed as the \`system="..."\` parameter directly in the \`messages.create()\` call.
...
a) Controls the speed of text generation
b) Controls the randomness/creativity of the response: low values give more predictable responses, high values more creative ones
c) Controls how many alternative responses the model generates
d) Controls the language of the response

**Correct answer: b)** — Temperature controls the probability distribution of tokens. With temperature 0.0, the model always picks the most probable token (deterministic). With high temperatures (1.0+), it explores less probable options, generating more varied and creative responses.
...
a) Reduces API cost
b) Improves the quality of the model's responses
c) Reduces perceived latency by showing tokens as they're generated
d) Allows sending longer prompts

**Correct answer: c)** — Streaming doesn't change quality or cost. Its advantage is that the first token arrives in ~200ms instead of waiting seconds for the entire response to be generated, which dramatically improves the user experience.
...
a) A type of database for storing texts
b) A numerical representation (vector) of the semantic meaning of a text
c) A text compression algorithm
d) A special format for sending prompts to the API

**Correct answer: b)** — An embedding converts text into a vector of numbers (for example, 1536 dimensions) that captures its semantic meaning. Texts with similar meanings produce vectors that are close in vector space.
...
**Correct answer: c)** — Cosine similarity measures the angle between two vectors, returning a value between -1 and 1. A value of 1.0 indicates identical vectors in direction (semantically identical texts), while 0.0 indicates no relationship.

---

### Question 8

Why should you never put an API key directly in source code?

a) Because it makes the code slower
b) Because if the code is uploaded to a public repository, anyone could use your API key and generate charges
c) Because APIs don't accept hardcoded keys
d) Because Python doesn't allow long strings in code

**Correct answer: b)** — API keys are secret credentials. If they're included in code and it gets uploaded to GitHub or another repository, automated bots can detect them within minutes and use them, generating enormous charges on your account. That's why \`.env\` files (excluded from Git with \`.gitignore\`) are used.
...
a) Vector databases generate higher quality embeddings
b) Vector databases are free while lists cost money
c) Vector databases use special indexes to find nearest neighbors efficiently, without comparing against every vector
d) Vector databases can store more types of data

**Correct answer: c)** — Searching a list requires calculating distance against every element (O(n)). Vector databases use structures like HNSW to perform approximate searches in O(log n), which is crucial when you have thousands or millions of documents.
...
a) The AI model internally stores each user's history
b) The complete list of previous messages (user + assistant) is sent in each API call
c) A vector database is used to store each message
d) A special prompt is used that says "remember the previous context"

**Correct answer: b)** — AI models have no memory between calls. "Memory" is implemented by maintaining a list of all previous messages (both user and assistant) and sending them complete in each new API call. That's why it's important to manage the history size so you don't exceed the model's token limit.
`;
